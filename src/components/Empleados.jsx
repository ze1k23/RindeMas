...
function FormEmpleado({ inicial, onGuardar, onCerrar }) {
  const vacio = { nombre: "", telefono: "", activo: true }
  const [form, setForm] = useState(inicial ? { ...inicial, telefono: inicial.telefono.replace(/[^0-9]/g, '') } : vacio)
    ...
    <Modal titulo={inicial ? "Editar empleado" : "Nuevo empleado"} onCerrar={onCerrar}
      ...
      {inicial && (
        <div className="flex items-center gap-3 pt-2">
          <label className="text-sm text-white/40">Activo</label>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-white/8 bg-white/3 overflow-hidden">
        <Tabla headers={["Nombre","Teléfono","Estado","Acciones"]}
