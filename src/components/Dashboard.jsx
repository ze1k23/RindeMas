...
import { formatPeso, calcCostoTrabajo, calcRentabilidadLote, estadoService } from "../utils"
...
export default function Dashboard({ onNavegar, userId }) {
  ...
  const lotesCosechados = lotes
    .map(l=>({lote:l,rent:calcRentabilidadLote(l,trabajos)}))
    ...
    <div className="space-y-5">
          ...
          <div className="space-y-3">
            {[
              ...
              <div key={p.num} className="flex items-center gap-3 text-sm">
                <span className="w-7 h-7 rounded-full bg-emerald-500/20 text-emerald-400 font-black text-xs flex items-center justify-center shrink-0">{p.num}</span>
                <span className="text-white/50">{p.text}</span>
                <button onClick={()=>onNavegar(p.link)} className="text-emerald-400 font-semibold hover:underline">{p.label} →</button>
              </div>
            ))}
          </div>
        </div>
      ...
      {urgentes.map(m=>(
        <div key={m.id} className="flex items-center gap-3 px-4 py-3 rounded-xl border bg-red-500/8 border-red-500/25 text-sm">
          ...
          <div className="flex-1 min-w-0">
            ...
            <span className="text-red-300/70 text-xs block sm:inline sm:ml-2">Service urgente — {m.horasService}h desde el último service</span>
          </div>
          <button onClick={()=>onNavegar("flota")} className="text-xs text-red-400 hover:underline shrink-0">Ver →</button>
        </div>
      ))}
      ...
      {!sinDatos && (
        <div className="grid grid-cols-2 gap-3">
          <KpiCard icon="🚜" label="Máquinas"     value={maquinas.length}
            sub={urgentes.length>0?`${urgentes.length} urgente`:alertas.length>0?`${alertas.length} próximo`:"todo OK"}
            color={urgentes.length>0?"text-red-400":alertas.length>0?"text-amber-400":"text-emerald-400"}/>
          <KpiCard icon="🌾" label="Hectáreas"    value={`${totalHa.toLocaleString()} ha`}
            sub={`${trabajos.length} trabajos`} color="text-sky-400"/>
          <KpiCard icon="📊" label="Costo/ha"     value={formatPeso(costoPorHa)}
            sub="promedio" color="text-violet-400"/>
          <KpiCard icon="💰" label="Ganancia neta" value={`${totalGanancia>=0?"+":""}${formatPeso(totalGanancia)}`}
            sub={`${lotesCosechados.length} lotes cosechados`}
            color={lotesCosechados.length===0?"text-white/30":totalGanancia>=0?"text-emerald-400":"text-red-400"}/>
        </div>
      )}
      ...
      <Seccion titulo="Últimos trabajos"
        ...
        {trabajos.length===0
          ? <div className="px-4 py-8 text-center text-white/25 text-sm">
              Sin trabajos. <button onClick={()=>onNavegar("trabajos")} className="text-emerald-400">Registrar →</button>
            </div>
                ...
                return (
                  <div key={t.id} className="px-4 py-3 flex items-center justify-between gap-3">
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-white/30 text-xs">{t.fecha} · {t.maquina}</p>
                    </div>
                    <p className="text-emerald-400 font-bold text-sm shrink-0">{formatPeso(cPorHa)}<span className="text-white/25 font-normal text-xs">/ha</span></p>
                  </div>
                )
              })}
            </div>
      ...
      <Seccion titulo="Estado de flota"
        accion={maquinas.length>0&&<button onClick={()=>onNavegar("flota")} className="text-xs text-emerald-400 hover:underline">Ver →</button>}>
        {maquinas.length===0
          ? <div className="px-4 py-8 text-center text-white/25 text-sm">
              Sin máquinas. <button onClick={()=>onNavegar("flota")} className="text-emerald-400">Agregar →</button>
            </div>
          : <div className="divide-y divide-white/5">
              {maquinas.map(m=>{
                  ...
                  <div key={m.id} className="px-4 py-3 flex items-center gap-3">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1.5">
                        ...
                        <span className="text-white/25 text-xs hidden sm:block">{m.marca}</span>
                      </div>
        ...
        <Seccion titulo="Rentabilidad por lote"
          ...
          <div className="divide-y divide-white/5">
            {lotesCosechados.map(({lote,rent})=>(
              <div key={lote.id} className="px-4 py-3 flex items-center justify-between gap-3">
                <div className="min-w-0">
                  <div className="flex items-center gap-2 mb-0.5">
                    <span className="text-white font-medium text-sm truncate">{lote.nombre}</span>
