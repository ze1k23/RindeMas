...
import { calcRentabilidadLoteDB, formatPeso, CULTIVOS, CATEGORIAS_GASTO } from "../utils"
import { getLotes, addLote, updateLote, deleteLote, getTrabajos } from "../db"

function FormLote({ inicial, onGuardar, onCerrar }) {
  const año = new Date().getFullYear()
  const vacio = { nombre:"", productor:"", cultivo:"Soja", ha:"", estado:"En preparación", campaña:`${año-1}/${año}` }
  const [form, setForm] = useState(inicial ? { ...inicial, ha: inicial.ha?.toString() } : vacio)
  ...
  return (
    <Modal titulo={inicial?"Editar lote":"Nuevo lote"} onCerrar={onCerrar}
      footer={<><BtnSecundario onClick={onCerrar}>Cancelar</BtnSecundario><BtnPrimario onClick={guardar}>{inicial?"Guardar cambios":"Crear lote"}</BtnPrimario></>}>
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <Campo label="Hectáreas"><input className={IC} type="number" min="0.1" placeholder="ej: 150" value={form.ha} onChange={e=>set("ha",e.target.value)}/></Campo>
        <Campo label="Campaña"><input className={IC} placeholder="ej: 2025/26" value={form.campaña} onChange={e=>set("campaña",e.target.value)}/></Campo>
      </div>
...
function FormCosecha({ lote, trabajos, onGuardar, onCerrar }) {
  ...
  const rent   = rinde>0 && precio>0 ? calcRentabilidadLoteDB({ ...lote, cosecha: { rinde, precioCereal: precio } }, trabajos) : null
  const guardar = () => {
  ...
  return (
    <Modal titulo="Registrar cosecha" subtitulo={`${lote.nombre} · ${lote.cultivo} · ${lote.ha} ha`} onCerrar={onCerrar}
      ...
      <div className="grid grid-cols-3 gap-4">
        <Campo label="Rinde (kg/ha)" hint="kilos por hectárea"><input className={IC} type="number" min="0" placeholder="ej: 3200" value={form.rinde} onChange={e=>set("rinde",e.target.value)}/></Campo>
        <Campo label="Precio cereal ($/kg)"><input className={IC} type="number" min="0" placeholder="ej: 380" value={form.precioCereal} onChange={e=>set("precioCereal",e.target.value)}/></Campo>
        <Campo label="Fecha de cosecha"><input className={IC} value={form.fecha} onChange={e=>set("fecha",e.target.value)}/></Campo>
      </div>
      {rent && !rent.sinCosecha && (
        <div className={`rounded-2xl p-4 border ${rent.rentable?"bg-emerald-500/5 border-emerald-500/20":"bg-red-500/5 border-red-500/20"}`}>
          <p className="text-xs text-white/30 uppercase tracking-wider mb-3 font-bold">Resultado completo del lote</p>
          <div className="grid grid-cols-2 gap-3 mb-3">
            <div><p className="text-xs text-white/30">Ingreso bruto/ha</p><p className="text-sky-400 font-bold text-xl">{formatPeso(rent.ingresoPorHa)}</p></div>
            <div><p className="text-xs text-white/30">Costo total/ha</p><p className="text-red-400 font-bold text-xl">{formatPeso(rent.costoPorHa)}</p></div>
          </div>
          <div className={`rounded-xl p-3 ${rent.rentable?"bg-emerald-500/10":"bg-red-500/10"} flex justify-between items-center`}>
            ...
            <div className="text-right"><p className="text-xs text-white/30 mb-1">Total del lote</p>
              <p className={`font-black text-xl ${rent.rentable?"text-emerald-400":"text-red-400"}`}>{rent.rentable?"+":""}{formatPeso(rent.gananciaTotal)}</p>
              <p className="text-white/25 text-xs mt-1">Margen: {rent.margen?.toFixed(1)}%</p></div>
          </div>
        </div>
      )}
    </Modal>
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-3">
          <button onClick={onVolver} className="p-2 rounded-xl text-white/35 hover:text-white hover:bg-white/8 transition-all text-sm font-bold">← Volver</button>
          <div>
            ...
            <p className="text-white/35 text-sm mt-0.5">{lote.cultivo} · {lote.ha} ha · {lote.productor} · Campaña {lote.campaña}</p>
          </div>
        </div>
        <div className="flex gap-2">
          <BtnIcono onClick={()=>setModalEditar(true)} icon="✏️" title="Editar lote"/>
          <BtnPrimario onClick={()=>setModalCosecha(true)}>🌾 {lote.cosecha?"Editar cosecha":"Registrar cosecha"}</BtnPrimario>
        </div>
      </div>
      ...
      {!rent.sinCosecha ? (
        <div className={`rounded-2xl border p-5 ${rent.rentable?"bg-emerald-500/5 border-emerald-500/20":"bg-red-500/5 border-red-500/20"}`}>
          ...
          <div className="flex gap-5 text-sm text-white/35 border-t border-white/8 pt-3 flex-wrap">
            ...
            <span>Combustible: <span className="text-amber-400 font-semibold">{formatPeso(comb.monto)} ({comb.trabajos} trab.)</span></span>
          </div>
        </div>
      ) : (
        <div className="rounded-2xl border border-amber-500/20 bg-amber-500/5 p-4 flex items-center gap-4">
          ...
          <button onClick={()=>setModalCosecha(true)} className="px-3 py-1.5 rounded-xl text-xs font-bold bg-amber-500 hover:bg-amber-400 text-white transition-all shrink-0">Registrar cosecha</button>
        </div>
      )}

      <Seccion titulo={`Costos por categoría — ${formatPeso(rent.costoTotal)} total`}>
        <div className="divide-y divide-white/5 px-5">
          {CATEGORIAS_GASTO.map(cat => {
            const gastosCat = (lote.gastos||[]).filter(g => g.categoria===cat.id)
            const total = gastosCat.reduce((s,g)=>s+(g.monto||0),0)
          ...
          {(lote.gastos||[]).length===0 && comb.monto===0 && (
            <p className="py-5 text-center text-white/25 text-sm">Los costos de insumos se calculan desde los trabajos registrados en este lote.</p>
          )}
        </div>
      </Seccion>
...
export default function Lotes({ userId }) {
  ...
  if (detalle) return <DetalleLote lote={detalle} trabajos={trabajos} onVolver={()=>setDetalle(null)} onActualizar={actualizarLote}/>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div><h2 className="text-xl font-bold text-white">Lotes</h2>
          <p className="text-sm text-white/30 mt-0.5">{lotes.length} lotes · {totalHa.toLocaleString()} ha totales</p></div>
      ...
      {lotes.length===0
        ? <div className="rounded-2xl border border-white/8 bg-white/3">
            <Vacio icon="🗺️" titulo="No hay lotes registrados" sub="Creá un lote para empezar a registrar tus costos y calcular la rentabilidad."
        ...
        : <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            {lotes.map(lote => {
              const rent = calcRentabilidadLoteDB(lote, trabajos)
              return (
                <div key={lote.id} className="rounded-2xl border border-white/8 bg-white/3 p-5 hover:border-white/15 transition-all cursor-pointer group" onClick={()=>setDetalle(lote)}>
                  <div className="flex items-start justify-between mb-4">
                    <div>
                      <p className="text-white font-bold text-lg group-hover:text-emerald-400 transition-colors">{lote.nombre}</p>
                      <p className="text-white/35 text-sm">{lote.productor} · {lote.cultivo} · {lote.ha} ha · Campaña {lote.campaña}</p>
                    </div>
                    <div className="flex items-center gap-2">
                      <Badge color={estadoColors[lote.estado]||"white"}>{lote.estado}</Badge>
                      <BtnIcono onClick={e=>{e.stopPropagation();setEliminar(lote)}} icon="🗑️" title="Eliminar" danger/>
                    </div>
                  </div>
                  <div className="grid grid-cols-3 gap-3 mb-4">
                    <div><p className="text-xs text-white/25 mb-0.5">Costo total</p>  <p className="text-red-400 font-semibold text-sm">{formatPeso(rent.costoTotal)}</p></div>
                    <div><p className="text-xs text-white/25 mb-0.5">Costo/ha</p>     <p className="text-white font-semibold text-sm">{formatPeso(rent.costoPorHa)}</p></div>
                    <div><p className="text-xs text-white/25 mb-0.5">Combustible</p>  <p className="text-amber-400 font-semibold text-sm">{formatPeso(rent.comb?.monto||0)}</p></div>
                  </div>
                  {!rent.sinCosecha
                    ? <div className={`rounded-xl p-3 flex justify-between items-center ${rent.rentable?"bg-emerald-500/8 border border-emerald-500/20":"bg-red-500/8 border border-red-500/20"}`}>
                        <div><p className="text-xs text-white/25 mb-0.5">Ganancia neta/ha</p>
                          <p className={`font-black text-xl ${rent.rentable?"text-emerald-400":"text-red-400"}`}>{rent.rentable?"+":""}{formatPeso(rent.gananciaPorHa)}</p></div>
                        <div className="text-right"><p className="text-xs text-white/25 mb-0.5">Margen</p>
                          <p className={`font-bold text-xl ${rent.rentable?"text-emerald-400":"text-red-400"}`}>{rent.margen?.toFixed(1)}%</p></div>
                      </div>
                    : <div className="rounded-xl p-3 bg-amber-500/5 border border-amber-500/15 flex items-center gap-2">
                        <span className="text-amber-400 text-sm">⏳</span>
                        <p className="text-amber-400/70 text-xs">Sin cosecha — clic para registrar</p>
                      </div>
                  }
                </div>
              )
      ...
      {modal    && <FormLote onGuardar={agregarLote} onCerrar={()=>setModal(false)}/>}
